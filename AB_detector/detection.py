...
from models.utils import set_logging,check_img_size,non_max_suppression,scale_coords, select_device
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    x1, y1, x2, y2 = float(xyxy[0].cpu()), float(xyxy[1].cpu()), float(xyxy[2].cpu()), float(xyxy[3].cpu())
                    if (y2-y1)/(x2-x1)<0.5 or  (y2-y1)/(x2-x1)>2:
                        continue
