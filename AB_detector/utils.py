...
def plot_one_box(x, img, label_location = 'up' ,color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        #c2 = (int(x[2]), int(x[3]))
        if label_location =='up':
            c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
            cv2.rectangle(img, c1, c2, color, -1)  # filled
            cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        elif label_location =='down':
            cv2.rectangle(img, (c1[0], c2[1] - 10), (c1[0]+t_size[0], c2[1] - t_size[1]-3), color, -1)  # filled
            cv2.putText(img, label, (c1[0], c2[1] - 2), 0, tl / 3, [255, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        else:
            c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
            cv2.rectangle(img, c1, c2, color, -1)  # filled
            cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
...
def count_overtime_trackid(frame_id, time_counter,track_id_coordinates, frame_counter, move_class, time_thresthold, lost_threshold, skip_frames):
    ...
    #print(move_class)
    for (track_id, class_time) in list(move_class.items()):
        print((track_id, class_time))
        #######
        return_coordinates.append(track_id_coordinates[track_id])
        if move_class[track_id]['all_move_num'] >= move_class[track_id]['all_stop_num']:
            move_stop_classes.append('moving-' + str(move_class[track_id]['stop_continuous_num']) + '-' + str(
                move_class[track_id]['all_move_num']) + '-' + str(track_id))
        else:
            move_stop_classes.append('stopping-' + str(move_class[track_id]['stop_continuous_num']) + '-' + str(
                move_class[track_id]['all_move_num']) + '-' + str(track_id))
        ######
        # if (class_time['stop_num']-1)*skip_frames+1 > time_thresthold:
        #     return_coordinates.append(track_id_coordinates[track_id])
        ...
        if (frame_id-frame_counter[track_id]-(time_counter[track_id]-1)*skip_frames)/skip_frames >= lost_threshold:
